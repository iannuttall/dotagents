...
export async function getMappings(opts: MappingOptions): Promise<Mapping[]> {
  ...
  const clients = new Set<Client>(opts.clients ?? ['claude', 'factory', 'codex', 'cursor', 'opencode', 'gemini', 'github', 'ampcode', 'kilocode']);
  ...
  mappings.push(
    {
      ...
      targets: [
        ...
        clients.has('kilocode') ? path.join(roots.kilocodeRoot, 'commands') : null,
      ].filter(Boolean) as string[],
      kind: 'dir',
    },
    {
